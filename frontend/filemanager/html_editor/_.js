...
html.prototype.treeDomEventListener = function(){
	...
	this.treeDom.addEventListener('dragover', (event) => {this.treeDOMDragover(event)});
	
	this.treeDom.addEventListener('dragenter', (event) => {this.treeDOMDragenter(event)});
	
	this.treeDom.addEventListener('drop', (event) => {this.treeDOMDrop(event)});
	
	$('#treeDom').delegate('li', 'dragstart', (event)=>{this.treeDOMDragstart(event.originalEvent);});
	
	$('#treeDom').delegate('li', 'dragend', (event)=>{this.treeDOMDragend(event);});
	
	$('#treeDom').delegate('li', 'click', (event)=>{this.treeDOMClick(event);});
	
	$('#treeDom').delegate('span', 'click', (event)=>{this.treeDOMSpanClick(event);});
	
	document.getElementsByTagName('body')[0].addEventListener('keydown', (event)=>{this.treeDOMKeyDown(event)});
}
